  ...
  if per06to12.to_i >= min_per || per12to18.to_i >= min_per || per18to24.to_i >= min_per
    word1 =
      ["いい朝だな！",
       ...
       "早起きして偉いぞ！",
       "いつもより起きるの遅いんじゃないのか？",
       "しっかり筋肉痛になっているか？"].sample
    word2 =
      ["今日もテストステロン全開でいこうぜ！",
       "雨だからジムが空いているぞ！チャンスだ！",
       "雨でもジムは休むなよ！",
       "カタボリックには気をつけような！",
       "トレーニング後のストレッチ、しっかりな！！"].sample
    # 降水確率によってメッセージを変更する閾値の設定
    mid_per = 50
    if per06to12.to_i >= mid_per || per12to18.to_i >= mid_per || per18to24.to_i >= mid_per
      word3 = ["今日は雨が降るかもしれないから傘とサラダチキン持って行くの忘れるなよ！",
              "今日は雨だからプロテイン持って行くの忘れるなよ、、、それと傘もな！"].sample
    else
      word3 = "雨が降るかもしれないから、トレーニンググッズのついでに折りたたみ傘も持っていこうな！"
    end
    # 発信するメッセージの設定
    push =
      "#{word1}\n#{word3}\n降水確率はこんな感じだよ。\n　  6〜12時　#{per06to12}％\n　12〜18時　 #{per12to18}％\n　18〜24時　#{per18to24}％\n#{word2}"
